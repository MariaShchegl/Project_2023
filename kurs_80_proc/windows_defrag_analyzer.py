...
def get_drives_letters():
    ...
    drives = list()
    os.system("chcp 1251 >> localhost") # использование в командной строке кодировки UTF-8
    os.system("echo list volume|diskpart >>" + diskpart_file) # запись в файл списка дисковых устройств
    for string in open(diskpart_file, "r", encoding='cp1251').readlines():
    ...
    while True:
        ...
        if key == "A":
            ...
            if result:
                ...
                if print_key == "y":
                    f = open(result, "r", encoding='cp1251')
                    os.system("cls")
                    print(f.read())
                    f.close()
